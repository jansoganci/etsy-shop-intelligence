import { getListingPerformance, type D1Database } from "../_performance";
...
export async function onRequestGet(context: Context): Promise<Response> {
  ...
  try {
    const performance = await getListingPerformance(context.env.DB, listingId);
    return Response.json({ ok: true, performance });
  } catch {
    return Response.json({ ok: false, error: "listing_performance_load_failed" }, { status: 500 });
  }
}
