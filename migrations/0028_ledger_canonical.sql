...
-- Makes the Etsy ledger readable as money: two read-only views, no table,
-- column or row change. Nothing consumes them yet (Stage 2 adds the endpoint
-- and the dashboard cards), so no existing figure moves.
--
-- WHY. The dashboard's Net Sales is `Gross - payment processing fee`, which is
-- also what Etsy's own screen shows — so the two agree. But that is not profit.
-- Advertising, the transaction commission, listing renewals and the VAT Etsy
-- charges on its own services never reach it. Measured on production D1
-- (2026-08-06) for 2026-05:
--
--   Dashboard "Net Sales"   $344.68
--   Actually kept            $187.88   (difference $156.80, 45%)
--
-- Every missing line already sits in etsy_api_ledger_entries (10,502 rows,
-- 2024-12-18 -> 2026-08-05). Until now nothing read that table as money; it
-- only fed the `ledger_complete` sync flag.
--
-- Data quality was verified before writing this: all 10,502 rows satisfy
-- `previous balance + amount = balance` (0 deviations), and the ledger's
-- PAYMENT_GROSS / PAYMENT_PROCESSING_FEE totals match etsy_api_payments and
-- the Etsy CSV exports to the cent.

-- ---------------------------------------------------------------------------
-- v_ledger_daily_rate — one TRY->USD rate per day, from Etsy's own figures
-- ---------------------------------------------------------------------------
-- Ledger amounts are in the settlement currency (TRY) and, unlike payments,
-- carry no per-row USD anchor: there is no receipt to divide by. So the rate is
-- derived per DAY using the same construction migration 0027 uses per payment —
-- settlement gross over the receipt's USD grandtotal, both Etsy's own numbers
-- for the same payments. No invented or averaged-across-periods rate.
--
-- The `amount_gross_divisor = 100` filter is load-bearing: payment
-- 204718752729 (buyer currency IDR) carries divisor 1 instead of 100 and would
-- skew its day's rate by 100x. That anomaly is documented in
-- migrations/0027_fix_payment_settlement_currency.sql.
...
-- ---------------------------------------------------------------------------
-- v_ledger_canonical — one row per ledger entry, categorised and USD-converted
-- ---------------------------------------------------------------------------
...
--
-- Self-checking invariant (a data-quality gate for consumers):
--
--   SUM(amount_try) over a period == closing balance - opening balance
--
-- Verified for 2026-05: 8,331.14 = 1,131.50 + 7,199.64.
...
SELECT
  ...
  CASE l.ledger_type
    ...
    ELSE 'other'
  END AS category,
  -- The day's own rate, or the most recent earlier day that had one. Ads are
  -- charged daily but some days have no sale (6 of 31 in 2026-05), so a
  -- carry-forward is required. TRY moves ~0.1%/day, so a one-to-three day
  -- carry costs cents. Entries before the first payment (2025-01-17) get NULL
  -- rather than a rate extrapolated backwards.
  (
    SELECT dr.settlement_per_usd
    FROM v_ledger_daily_rate dr
    WHERE dr.rate_date <= date(l.create_timestamp, 'unixepoch')
